...
def main(preset_args = False):
    ...
    validation_exists = (len(corpus.splits) >= 3)
    # set annealing to false if we're supposed to train on the validation set
    #     but there is no validation set
    if not validation_exists and annealing == True and args.anneal_train == False:
        annealing = False

    # flatten the data and move it to GPU
    for d in corpus.splits:
    ...
    def repackage_hidden(h):
        ...
        return h if type(h) != Variable else tuple(repackage_hidden(v) for v in h)
    ...
    def evaluate(data_source):
        ...
        return total_loss / len(data_source['lengths'])
    ...
    def train(optimizer):
        ...
        for word_index, word_len in enumerate(corpus.train['lengths']):
            ...
            hidden = model.init_hidden() # reset model statei
            #print(hidden[0].data)
            #print("SHAAAAAAAPE: ", hidden[0].shape)
            #print("TYYYYYYYYYPE:", type(hidden[0]))
            ...
            #print(loss.data)
            #print(loss.data.shape, "this is the loss data shape")
            #print(loss.data.type, "this is the loss data type")
    ...
    try:
        for epoch in range(1, args.epochs+1):
            ...
            if validation_exists:
                ...
                print('| end of epoch {:3d} | time: {:5.2f}s | no validation set'.format(epoch, output_info['epoch_time']))
            print('-' * 89)
            ...
            if not validation_exists or not best_val_loss or val_loss < best_val_loss:
