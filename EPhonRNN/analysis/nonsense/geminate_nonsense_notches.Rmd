...
```{r}
ggplot(eval_data) +
  geom_boxplot(aes(x=n_obs, y=loss_mean, color=gem), notch=TRUE)+
  xlab("N. of voiced obstruents in the word")+
  ylab("Average log  likelihood")+
   ggtitle("Average log likelihoods per geminate voicing \n and n. of obstruents in the word\n Word set: test (55366 nonsense words)")+
  scale_color_brewer(name = "Geminate voicing", palette="Dark2") 
  

ggplot(dist_data) +
  geom_boxplot(aes(x=dist, y=loss_mean, color=gem), notch=TRUE)+
  xlab("N. of voiced obstruents in the word")+
  ylab("Average log  likelihood")+
